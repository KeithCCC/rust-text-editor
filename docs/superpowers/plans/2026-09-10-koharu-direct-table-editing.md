...
> **For agentic workers:** REQUIRED SUB-SKILL: Use superpowers:executing-plans to implement this plan task-by-task. Steps use checkbox (`- [ ]`) syntax for tracking. この文書は実装計画であり、今回の作業ではアプリケーションを変更しない。
...
1名で実装する場合の目安は **8〜13実働日**。1〜2に2〜3日、3に2〜3日、4〜5に2〜3日、6〜7に2〜4日を見込む。実機 IME と履歴・DOM 更新の相性は未検証なので、Task 3 完了時に見積もりを更新する。これは調査段階の目安であり、納期の確約ではない。
